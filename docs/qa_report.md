...
## `/tickets-rental` 差分再評価 — 2026-06-11

> **Agent**: `resort-qa-a11y`（L3）
> **Target**: `resorts/Sichinohe-CyoueiSki/web/src/app/[locale]/tickets-rental/`
> **L2 変更**: `AwardFold` アコーディオン撤去 → `AwardContentSection`（静的 h3 セクション・テーブル常時表示）; `AwardFold` 本体改善（`summary min-height`・`focus-visible`・円形マーカー）; `award-design-system.css` 対応追記

### 変更確認

| 要件 | 判定 | 根拠 |
|------|------|------|
| `AwardFold` アコーディオン撤去 | PASS | `page.tsx` は `AwardContentSection` のみ使用。`AwardFold` インポートなし |
| 表が常時表示 | PASS | `AwardContentSection` にはトグルなし。`<table>` は DOM に直置き |
| 新規 `AwardContentSection` 構造 | PASS | `<section class="award-content-section"> <h3> + <div class="...body">` |
| `AwardFold` 改善（FAQ 等で継続使用） | PASS | `award-fold__summary min-height: 3.25rem`（52px）・`:focus-visible` outline・マーカー円形化 |
| `award-design-system.css` 追記 | PASS | `.award-content-section` / `__title` / `__body` スタイル追加。既存クラスとの衝突なし |
...
| 項目 | 判定 | 根拠 |
|------|------|------|
| タッチターゲット ≥ 44px（tickets-rental） | PASS | このページにインタラクティブ要素なし（静的テーブル）。`notice-banner` は `<p>` |
| タッチターゲット ≥ 44px（AwardFold — FAQ 等） | PASS | `award-fold__summary min-height: 3.25rem` = 52px。`award-fold__marker`（40px）は `aria-hidden` で interactive 面積は summary 全体 |
| 375px 横スクロールなし | PASS | `.award-table { width: 100% }` + `award-inner padding-inline: clamp(1.25rem, 4vw, 3rem)`。テーブルセルは `white-space: normal` でラップ |
| **非ブロッカー（P2）** | WARN | `award-table` に `overflow-x: auto` スクロールラッパーなし。データ文字列が非常に長い場合の保険として追加推奨 |

### Q2 Accessibility — **PASS**

| 項目 | 判定 | 根拠 |
|------|------|------|
| 見出し階層 | PASS | `SectionHeader` → `EditorialTitle as="h2"` がページ h2。`AwardContentSection` が `<h3>`。h2 → h3 正しい階層 |
| テーブル `aria-label` | PASS | 3 テーブル全てに `aria-label={t("tableAria.*")}` 付与（ja/en 両対応） |
| テーブル `th scope` | PASS | 全 `<th>` に `scope="col"` ✅ |
| `AwardFold` フォーカスリング | PASS | `.award-fold__summary:focus-visible { outline: 2px solid var(--award-color-accent); outline-offset: 2px }` |
| `AwardFold` マーカー | PASS | `award-fold__marker` は `aria-hidden="true"`。+/− 記号は読み上げ対象外 |
| `<details>` 開閉状態 | PASS | `<details open>` の `open` 属性変化をブラウザが `expanded`/`collapsed` として読み上げ（ネイティブ） |
| `notice-banner` | PASS | `<p role="note">` — コンテンツは `getResortData()` から取得 |
| `prefers-reduced-motion` | PASS | `.award-rise` / `.award-btn-*:hover` transform を `@media (prefers-reduced-motion: reduce)` で無効化済み |
| **非ブロッカー（P2）** | WARN | `<section class="award-content-section">` に `aria-labelledby` が未設定。`<h3>` に `id` を付けて紐付けると WCAG 2.4.6 advisory 準拠 |

### Q3 Conversion path — **PASS**

| 導線 | 評価 |
|------|------|
| 料金情報へのアクセス | PASS — テーブル常時表示により「展開タップ」不要。情報到達が 1 ステップ短縮 |
| サイト内主要導線 | PASS — `SiteHeader` 経由 ≤ 3 タップ（変更なし） |

アコーディオン撤去により情報発見性が向上。ブロッカーなし。

### Q4 i18n — **PASS**

| 項目 | 判定 | 根拠 |
|------|------|------|
| `messages/ja.json` `ticketsRental.*` | PASS | `title` / `eyebrow` / `shellDescription` / `ticketsSection` / `rentalSection` / `paymentSection` / `cols.*` / `tableAria.*` 全キー確認 |
| `messages/en.json` `ticketsRental.*` | PASS | 同構造・全キー対応確認（`tableAria.rentals: "Rental equipment"` 等） |
| コンポーネント内ハードコード JA | PASS | `AwardContentSection` / `AwardFold` に固定文字列なし |
| 動的データ | PASS | `notice` / `ticketRows` / `rentalRows` / `paymentRows` は `getResortData()` から取得。locale 別 `resort-data.json` / `resort-data.en.json` |
| `generateMetadata` | PASS | `t("title")` + `t("description")` — locale 対応済み |

### Q5 Performance — **PASS**

| 項目 | 判定 | 根拠 |
|------|------|------|
| ヒーロー画像 | N/A | このページに画像なし |
| アニメーションループ | PASS | `award-rise` は一発 entry アニメ（ループなし）+ reduced-motion ガード済み |
| クライアント JS | PASS | `AwardContentSection` / テーブルはサーバー側 RSC。`AwardFold` は `<details>` ネイティブ（JS 不要） |

### Q6 Data separation — **PASS**

| 項目 | 判定 | 根拠 |
|------|------|------|
| 価格データ | PASS | `ticketRows`（`row.price`）は `resort-data.json` — `messages/*.json` に価格なし |
| UI ラベル | PASS | 列ヘッダー・セクション名は `messages/*.json` のみ |
| ロケール間重複なし | PASS | `tableAria.*` / `cols.*` は UI 文言のみ。数値・状況フラグはデータ層 |
...
1. `award-table` に `overflow-x: auto` スクロールラッパー未設定 — 現データでは問題ないが長文データへの保険として推奨
2. `AwardContentSection` の `<section>` に `aria-labelledby` 未付与 — WCAG 2.4.6 advisory（ランドマーク明示）
3. `award-fold__marker` の寸法が 40px（`2.5rem`）— タッチ target は `summary`（52px）全体のため非ブロック

### 総合判定: **PASS**

```
resort-qa-a11y PASS（/tickets-rental 差分 2026-06-11）+ resort-visual-evaluator PASS → /tickets-rental 出荷可
```

**注**: a11y PASS のみでは出荷不可。ビジュアル再評価（`resort-visual-evaluator`）は別途必須。
